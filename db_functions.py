...
def check_user_pay(code):
    try:
        ...
        row = list(row[0])[2]
        if row != 0:
            st = """С твоим статусом все отлично, желаем хорошо оторваться на дискотеке!✌️"""
        else:
            st = """На данный момент ты не можешь пройти на дискотеку, в ближайшее время найди организаторов, загляни в  /info"""
        return st
    except IndexError:
